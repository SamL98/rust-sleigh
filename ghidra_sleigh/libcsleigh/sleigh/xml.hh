...
protected:
  ...
  // vector<rapidxml::xml_attribute<> *> attrs;
public:
  Element(rapidxml::xml_node<> *n) {
    name = n->name();
    content = n->value();
    ...
    for (rapidxml::xml_node<> *c = n->first_node(); c; c = c->next_sibling()) {
      auto elem = new Element(c);
      elem->parent = this;
      children.push_back(elem);
    }

    for (rapidxml::xml_attribute<> *a = n->first_attribute(); a; a = a->next_attribute()) {
      attr.push_back(string(a->name()));
      value.push_back(string(a->value()));
    }
  }
...
public:
  ...
  Document(rapidxml::xml_node<> *doc) : Element(doc) {}
  Element *getRoot(void) const { return (Element *)this; }	///< Get the root Element of the document
};
